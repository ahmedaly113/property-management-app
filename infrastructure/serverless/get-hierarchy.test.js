...
describe('getHierarchy', () => {
  ...
  it('integer fields enrichment', done => {
    ...
    .then(res => {
      ...
      const refWeightFacet = facets[0]
      expect(refWeightFacet.validation.isRegEx).to.eql(true)
      expect(refWeightFacet.validation.value).to.eql('^[0-9]+$')
      expect(refWeightFacet.validation.errorMessage).to.eql('Must be a number')
      expect(refWeightFacet.validation.example).to.eql('5')
      done()
    })
    .catch(e => {
      done(new Error(e.message))
  ...
  it('rating fields test', done => {
    ...
    .then(res => {
      ...
      expect(ratingFacet.key).to.eql('rating-kW')
      expect(ratingFacet.label).to.eql('Rating (kW)')
      expect(ratingFacet.help).to.eql('kW')
      expect(ratingFacet.group).to.eql('Others')
      expect(ratingFacet.ratingGroup).to.eql('Power')
      expect(ratingFacet.element).to.eql('rating')
      done()
    })
    .catch(e => {
      done(new Error(e.message))
