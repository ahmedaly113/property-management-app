...
export default class FilterByButtonTestHelper {
  ...
  calcFilteredTreeDataFromAssetsList() {
    ...
    this.mockedProps.assets = this.assets
  }
  ...
  mockAssetsListWithRating() {
    this.assets = [
      {
        type: 'Electrical-meter-002',
        rating: {
          name: 'Rating (A)',
          type: 'Current',
          key: 'rating-A',
...
Given('an assets list with rating', () => {
  object.mockAssetsListWithRating()
  object.calcFilteredTreeDataFromAssetsList()
})

Then('the filter should contain rating', () => {
  pageObject.checkIncludeRating()
})

When('user click rating filter', () => {
  pageObject.simulateAddingRatingFilter()
})
