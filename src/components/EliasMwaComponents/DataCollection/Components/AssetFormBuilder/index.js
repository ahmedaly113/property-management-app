...
class AssetFormBuilder extends React.Component {
  ...
  handleSubmit = (values, allowByPass) => {
    ...
    Object.keys(newFormData).forEach(obj => {
      ...
      } else if (obj.startsWith('rating.')) {
        const { schema } = values
        const ratingSchema = schema.properties.Others.properties
        const ratingData = ratingSchema[obj]
        data.rating = {
          name: ratingData.title,
          type: ratingData.ratingGroup,
          key: ratingData.key.replace('rating.', ''),
          unit: ratingData.help,
          value: newFormData[obj],
        }
      } else {
