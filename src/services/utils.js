...
const fitOneCondition = (object, key, filter) => {
  ...
  if (
    !filter.find(filterItem => {
      ...
      if (key === 'rating') {
        const ratingUnit = object.rating.unit
        return ratingUnit && filter.indexOf(ratingUnit) !== -1
      }
...
export const filterAssetRatingTreeData = assets => {
  const treeData = ['Power', 'Current', 'Area', 'Mass', 'Volume', 'Pressure'].map(groupTitle => ({
    key: groupTitle.toLowerCase(),
    selectable: false,
    title: groupTitle,
    value: groupTitle.toLowerCase(),
    children: [],
  }))

  let filteredTreeData = cloneDeep(treeData)

  if (Array.isArray(filteredTreeData) && assets && assets.length !== 0) {
    assets.forEach(asset => {
      const assetRating = asset.rating || {}
      const ratingGroup = filteredTreeData.find(item => item.title === assetRating.type)
      if (!ratingGroup) {
        return
      }
      if (!ratingGroup.children.find(item => item.key === assetRating.key)) {
        ratingGroup.children.push({
          key: assetRating.key,
          selectable: true,
          title: assetRating.name,
          value: assetRating.key,
        })
      }
    })
    filteredTreeData = filteredTreeData.filter(tD => tD.children.length)
  } else {
    filteredTreeData = []
  }
...
export const sort = (array, sortBy, ascending) =>
  [...array].sort((a, b) => {
    ...
    if (typeof get(a, sortBy, 0) === 'number' && typeof get(b, sortBy, 0) === 'number') {
      return ascending
        ? get(a, sortBy, 0) - get(b, sortBy, 0)
        : get(b, sortBy, 0) - get(a, sortBy, 0)
    }

    return ascending
      ? get(a, sortBy, '').localeCompare(get(b, sortBy, ''))
      : get(b, sortBy, '').localeCompare(get(a, sortBy, ''))
  })
